...
def test_property_descriptor():
    ...
    Procedure:
        ...
        - Create a list of field types that include defined classes, undefined 
          classes and datatypes.
        - Create a property descriptor from the class_list and field_list.
        - Create multiline comments to be added.
        - Call to_proto function along with comments to get the protobuf code 
          in string format.
    ...
    p = property_descriptor.PropertyDescriptor(
        property_name, field_types, class_list)
...
def test_class_descriptor():
    ...
    Procedure:
        ...
        - The field type shud be mapped to their parents using 
          utils.PropertyToParent class.
    ...
    Verification:
        ...
        - Check if the class has a (type) option with value as the name of the
          class.
        - Check if multiline comments are properly formatted.
        - Check if the properties that are not inherited appear before the 
          properties that are inherited.
        - Check if the all the properties that belong to same parent appear 
          together..
        - Check if all the properties are repeated fields.
        - Check if every property have (json_name) option with value as the 
          name of property.
    """
...
def test_enum_descriptor():
    ...
    Verification:
        ...
        - Check if the nested enum defenition has all the values in enum_values
          along with default unknown value,
        - Check if the every value in enum definition has a option 
          (schemaorg_value) pointing to schem.org url.
        - Check the following for first message
            * Check if the first field is id and is a singular field.
            * Check if the message has a (type) option with value as the name 
              of the class.
            * Check if multiline comments are properly formatted.
            * Check if the properties that are not inherited appear before the
              properties that are inherited.
        ...
        - Check the following for second message.
            ...
            * Check if the message has a (type) option with value as
              'EnumWrapper'.
            * Check if all fields are enclosed inside a oneof.
    """
    class_name = 'Foo'
