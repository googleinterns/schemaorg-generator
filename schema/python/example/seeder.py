...
class IMDBSeeder():
    """The IMDBSeeder populates the database with data that is required for
    running IMDBExample class.
    ...
    def __init__(self, cursor, db_name):
        ...
        movie_table = """CREATE table movie (
            url varchar(200) primary key,
            name varchar(200), image varchar(200),
            ...
            rating_count int,
            rating float(4,2),
            best_rating float(4,2),
            worst_rating float(4,2)
            );"""

        series_table = """CREATE table series (
            url varchar(200) primary key,
            name varchar(200), image varchar(200),
            content_rating varchar(10),
            description text,
        ...
        episode_table = """CREATE table episode (
            url varchar(200) primary key,
            name varchar(200),
            image varchar(200),
            content_rating varchar(10),
        ...
        genres_table = """CREATE table genre (
            entity_url varchar(200),
            name varchar(200)
            );"""

        actor_table = """CREATE table actor (
            entity_url varchar(200),
            url varchar(200),
            name varchar(200)
            );"""

        director_table = """CREATE table director (
            entity_url varchar(200),
            url varchar(200),
            name varchar(200)
            );"""

        creator_table = """CREATE table creator (
            entity_url varchar(200),
            url varchar(200),
            name varchar(200),
        ...
        trailer_table = """CREATE table trailer(
            name varchar(200),
    ...
    def seed_db(self, cursor, movie_list, series_list, episode_list):
        """Call __seed_movies, __seed_series, __seed_episodes and seed the
        database .
        ...
        for m in movie_list:
            ...
            if 'director' in m:
                if isinstance(m['director'], list):
                    ...
                    self.__seed_director(
                        cursor, m['director'], m.get(
                            'url', ''))
    ...
    def __seed_review(self, cursor, review, url):
        ...
        if 'reviewRating' in review:
            ...
            query = """INSERT INTO review
                (entity_url, author, date_created, language, name, review_body)
                values ({}, {}, {}, {}, {}, {})""".format(
                json.dumps(url),
                json.dumps(review['author'].get('name', '')),
                json.dumps(review.get('dateCreated', '')),
